<h2>https://usaihack.github.io/Portfolio/</h2>

# 👋 Hey, I'm Usman Said  

💻 **Cybersecurity & Programming Enthusiast**  
Exploring the layers of systems, networks, and code, one exploit, one fix at a time.  
Curiosity drives me. Precision grounds me.  

---

### 🧠 About Me
- 🔐 Passionate about **ethical hacking**, **CTFs**, and **cyber defense**.  
- 🧩 Learning **network security**, **scripting**, and **system internals**.    
- 🌱 Constantly experimenting and documenting the process.  

---



### ⚡ Tools & Tech
`Python` `Bash` `Linux` `Networking` `Wireshark` `Nmap` `Burp Suite` `Metasploit` `TryHackMe` `Git`

---

### 🌐 Connect with Me
[💼 LinkedIn](https://linkedin.com/in/yourusername)  
[🐙 GitHub](https://github.com/usaihack)  
[🎯 TryHackMe](https://tryhackme.com/p/yourusername)  
[💬 Discord](https://discord.com/users/usaihack)  

---

> “Security is not about breaking systems, it’s about understanding them.”  
>  Usman Said
