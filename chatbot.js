...
const responses = {
  ...
  projects: [
    ...
    "ðŸ“˜ Notes â€” Daily Learning Notes in an organized HTML format",
  ],
...
function startContactFlow() {
  ...
  clearOldButtons();
  botReply("ðŸ“ Let's send Usman a message!\n\nFirst â€” what's your name?", false);
}
...
function sendContactMessage() {
  ...
  sendingDiv.textContent = "ðŸ“¡ Sending your message";
  ...
  const dotAnim = setInterval(() => {
    dots = (dots + 1) % 4;
    sendingDiv.textContent = "ðŸ“¡ Sending your message" + ".".repeat(dots);
  }, 400);
  ...
  function finish(success) {
    ...
    if (success) {
      typeText(
        ...
        "He personally reads and replies to every message â€” usually within 24â€“48 hours. ðŸš€\n\n" +
        "Check your inbox for a confirmation email!",
        true
      );
    } else {
      typeText(
        "âŒ Message failed to send â€” possibly a network issue.\n\n" +
...
function handleUserInput(input) {
  ...
  if (msg === "about" || msg === "bio")               botReply(formatText(responses.about));
    ...
    if (quotes.length === 0) botReply("âš ï¸ No more quotes until refresh.");
      ...
      botReply("ðŸ’¬ " + quotes.splice(i, 1)[0]);
    }
  }
